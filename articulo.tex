...
La formulación simplificada $F(t) = \min_{0 \leq j < t} [F(j) + s_{j+1} + H(j,t)]$ es equivalente a resolver la ecuación de Bellman (\ref{eq:dp_bellman}) bajo las propiedades estructurales de Wagner-Whitin, donde $H(j,t)$ representa el costo de mantener inventario al cubrir demandas de $j+1$ a $t$ con una orden en $j+1$.
\end{remark}
...
x_3 &= 0, \quad I_3 = 30
\end{align*}
\emph{Nota: Los niveles de inventario se calculan correctamente: $I_1 = 120-20=100$, $I_2=100-30=70$, $I_3=70-40=30$}
...
Pero esta situación viola directamente la \textbf{Propiedad de Cero Inventarios} del Teorema 4.1, ya que tenemos $I_{m-1} \cdot x_m > 0$.
...
El modelo propuesto por Richter y Weber (2001) extiende el algoritmo Wagner-Whitin tradicional al incorporar:

\begin{itemize}
    \item \textbf{Dos fuentes de suministro}: Remanufactura de productos usados y manufactura tradicional
    \item \textbf{Dos tipos de inventario}: Productos finales y productos usados
    \item \textbf{Costos variables}: Tanto para manufactura como para remanufactura
    \item \textbf{Propiedad de monotonicidad}: En condiciones óptimas, la remanufactura precede a la manufactura
\end{itemize}

Este modelo es particularmente relevante en contextos de economía circular y sostenibilidad, donde la recuperación y reutilización de productos adquiere importancia creciente.
